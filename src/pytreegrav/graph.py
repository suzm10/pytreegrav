"""Implementation of the HNSWGraph jitclass"""
...
spec = [
    ...
    ("M", int64),
    ("N", int64), # num particles
    ("MaxNumLevels", int64),
    ("MaxNumChildren", int64),
    ("NumLevels", int64),
    ("AdjacencyList", int64[:, :]),
    ("DownwardLinks", int64[:, :])
]
...
class HNSWGraph:
    def __init__(
        ...
        for l in range(self.MaxNumLevels):
            ## cluster nodes
            ...
            numCentroids, assignments = ClusterNodes(
                self.Coordinates[currLayerNodeIdxs],
                currLayerNodeIdxs,
                self.M
            )
            # print(f"l: {l}")
...
def spread_bits(x):
    """Spreads 21 bits of x into every 3rd bit of a 64-bit integer."""
    x &= 0x1fffff
...
@njit(int64(float64[:], float64[:], float64), fastmath=True)
def get_morton_code(pos, min_bound, scale):
    """
    Computes a 64-bit Morton code for a 3D point.
    pos: (3,) float array
    min_bound: (3,) float array (minimum x,y,z of the system)
    scale: float (1 / max_range of the system)
    """
    # Normalize and scale to 21-bit integer range (0 to 2,097,151)
    ix = int((pos[0] - min_bound[0]) * scale * 2097151)
    iy = int((pos[1] - min_bound[1]) * scale * 2097151)
    iz = int((pos[2] - min_bound[2]) * scale * 2097151)
    
    return spread_bits(ix) | (spread_bits(iy) << 1) | (spread_bits(iz) << 2)

@njit(fastmath=True)
def ClusterNodes(coords, idxs, M):
    N = len(coords)
    codes = np.empty(N, dtype=int64)

    if len(coords) != len(idxs):
        print("why don't lengths match")
        return
    if N == 0:
        return 0, np.empty(0, dtype=int64)

    min_b = np.array([np.min(coords[:,0]), np.min(coords[:,1]), np.min(coords[:,2])])
    max_b = np.array([np.max(coords[:,0]), np.max(coords[:,1]), np.max(coords[:,2])])
    scale = 1.0 / max(1e-10, np.max(max_b - min_b))

    for i in range(N):
        codes[i] = get_morton_code(coords[i], min_b, scale)
    
    local_sort_idx = np.argsort(codes)
    # sorted_idxs = idxs[np.argsort(codes)]

    numCentroids = int(np.ceil(N / M))
    assignments = np.empty(N, np.int64)

    for i in range(N):
        # assignments[sorted_idxs[i]] = i // M
        original_local_idx = local_sort_idx[i]
        assignments[original_local_idx] = i // M

    return numCentroids, assignments
...
#     ):
#         self.NumNodes = 0
#         children = self.BuildGraph(points, masses, softening)

#     def BuildGraph(self, points, masses, softening):
#         np.random.seed(42)
...
#         self.M = 16
#         self.MaxNumLevels = int(np.ceil(np.log(Npart) / np.log(self.M))) + 2
#         self.MaxNumChildren = 4*self.M
...
#         self.Softenings = zeros(self.NumNodes)
#         self.Coordinates = zeros((self.NumNodes, 3))
#         self.Levels = zeros(self.NumNodes)
#         self.Children = -ones(self.NumNodes, self.MaxNumChildren)
